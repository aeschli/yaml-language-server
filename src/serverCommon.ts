...
import {
    createConnection, Connection, TextDocuments, InitializeParams, InitializeResult,
    Disposable, ProposedFeatures, CompletionList, ClientCapabilities, WorkspaceFolder, DocumentFormattingRequest, TextDocumentSyncKind
} from 'vscode-languageserver';

import { TextDocument } from 'vscode-languageserver-textdocument';

import { XHRResponse, XHROptions } from 'request-light';
import * as URL from 'url';
import { removeDuplicatesObj } from './languageservice/utils/arrUtils';
import { getLanguageService as getCustomLanguageService, LanguageSettings, CustomFormatterOptions, WorkspaceContextService, SchemaRequestService } from './languageservice/yamlLanguageService';
...
const workspaceContext: WorkspaceContextService = {
    resolveRelativePath: (relativePath: string, resource: string) => { return URL.resolve(resource, relativePath); }
};
...
export interface RuntimeEnvironment {
    schemaRequestHandler: SchemaRequestService;
    schemaStoreRequestHandler(uri: string): Promise<string>;
...
export function startServer(connection: Connection, runtime: RuntimeEnvironment) {
    ...
    function getSchemaStoreMatchingSchemas() {
        return runtime.schemaStoreRequestHandler(JSON_SCHEMASTORE_URL).then(response => {
    ...
    const schemaRequestService = (uri: string) => {
        // If the requested schema URI is a relative file path
        // Convert it into a proper absolute path URI
        if (uri && isRelativePath(uri)) {
            uri = relativeToAbsolutePath(workspaceFolders, workspaceRoot, uri);
        }
        return runtime.schemaRequestHandler(uri);
    }

    const customLanguageService = getCustomLanguageService(schemaRequestService, workspaceContext, []);
    ...
    connection.onDefinition(params => {
        ...
        if (clientDefinitionLinkSupport) {
            ...
            return definitionLinksPromise.then(definitionLinks => {
                return definitionLinks.map(definitionLink => { return { uri: definitionLink.targetUri, range: definitionLink.targetRange }; });
            });
        }
    });
